    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                result += getWordScore(guess, n)
                print('"'+guess+'"', "earned", getWordScore(guess, n),
                      "points. Total:", result, "points", '\n')
                # Update the hand
                hand = updateHand(hand, guess)

    # Game is over (user entered a '.' or ran out of letters), so tell user the total score
    if guess == '.':
       print('Goodbye! Total result:', result, 'points.')
    else:
        print('Run out of letters. Total result:', result, 'points.')
